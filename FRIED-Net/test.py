...
for i, file in enumerate(args.data_filename):
    ...
    if not args.overwrite and os.path.exists(output_path):
        ...
        print("Loaded results from {}".format(output_path))
    else:
        ...
        if args.fine_tune:
            ...
            for i, batch in enumerate(testing_data_loader):   # Fine-tune for each batch of test data
                ...
                for i in range(args.num_epochs):
                    optimizer.zero_grad()
                    y_recon, t_k_est_batch = model(y_n_noisy, _, _)
        ...
        sio.savemat(output_path, {'t_k_est': t_k_est})
    ...
    print('Ground Truth: {}'.format(t_k if t_k.ndim == 1 else t_k[data_idx]))
    ...
    try:
        sd_t_k_est = np.sqrt(mean_squared_error(np.tile([t_k], (t_k_est.shape[0], 1)) if t_k.ndim == 1 else t_k, t_k_est, multioutput='raw_values'))
    except ValueError:
        sd_t_k_est = np.sqrt(mean_squared_error(np.tile([t_k], (t_k_est.shape[0], 1)) if t_k.ndim == 1 else t_k, np.nan_to_num(t_k_est), multioutput='raw_values'))
